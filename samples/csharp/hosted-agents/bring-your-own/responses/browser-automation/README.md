...
azd ai agent invoke --local \
  ...
  --protocol responses \
  --new-session \
  "/verbose Navigate to https://example.com using the browser, inspect the page, and report its exact page title and main heading."
```

A successful response includes `run_browser` activity and reports `Example
...
- **`playwright-cli` is not found:** Install the CLI and ensure its executable
  directory is on `PATH`. On Windows, the sample resolves the npm `.cmd` shim
  and runs it through `cmd.exe`; on Linux and macOS it uses the executable shim.
