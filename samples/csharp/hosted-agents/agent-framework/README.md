...
Local Responses API requests require a stable user identity for session isolation.
Use `--user-identity` with `azd`, or send the equivalent `x-agent-user-id` header
with an HTTP client.
