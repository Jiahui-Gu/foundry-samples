...
Local requests must include a user identity so the agent can isolate session state. In a separate terminal, invoke the running agent with a stable local-only identity:
